...
source_object::source_object(std::filesystem::path in_path,
  ...
  spdlog::info("New SourceObject: {}", path.string());
}

std::pair<int, int> byteOffsetToLineColumn(const std::string_view text,
...
std::vector<language::class_declaration> source_object::get_classes() const {
  const std::string_view class_name_query_source =
      "[(class_specifier name: (type_identifier) @name body: _)"
  ...
  for (const ts::QueryMatch &match : cursor) {
    for (const auto &capture : match.getCaptures()) {
      ...
      if (range.start >= content.size() || range.end > content.size() ||
      ...
      result.emplace_back(class_name,
                          language::location{.file = path,
                                             .line = start_point.first,
                                             .column = start_point.second});
    }
  }
